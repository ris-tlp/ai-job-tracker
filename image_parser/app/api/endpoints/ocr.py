...
from app.exceptions.ocr import OCRServiceError, ImageProcessingError, UnsupportedImageError
from app.schemas.ocr import TextResponse, ErrorResponse
...
async def extract_text(
    ...
    if not file.content_type or not file.content_type.startswith('image/'):
        error_msg = f"Unsupported file type: {file.content_type}. Please upload an image file."
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Unsupported image: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "error": str(e),
            ...
            logger.error(f"OCR service error: {str(e)}", exc_info=True)
            raise HTTPException(
        ...
        try:
            ...
            logger.error(f"Error closing file: {str(e)}", exc_info=True)